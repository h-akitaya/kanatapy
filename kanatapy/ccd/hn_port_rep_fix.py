...
def port_replacement_fix(fn_in: str, fn_out: str,
                         overwrite: bool=False) -> np.ndarray:
    """
    fn_in: input fits file.
    fn_out: output fits file.
    """
    # Output file and overwritemode check.
    if os.path.exists(fn_out) and overwrite == False:
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(fn_in):
        print('File {} not found.'.format(fn_in))
        sys.exit(1)
    ...
    # Main routine.
    data = port_replacement_fix(fn_in, fn_out, overwrite=args.overwrite)
    
    if args.verbose:
        print('Fix. {} -> {}'.format(fn_in, fn_out))
